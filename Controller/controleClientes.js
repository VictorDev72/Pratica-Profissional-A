...
async function criaCartao(req, res) {
    try {
        const info = req.body
        const result = await clienteReq.cadastroCartao(info)
        res.status(201).json(result);
    } catch (error) {
        res.status(400).json({ error: "erro ao inserir cartao",error })
    }
}

module.exports = {cadastrado, login, criaCartao}
